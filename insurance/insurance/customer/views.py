...
from admins import models as CMODEL
from admins import forms as CFORM
...
def apply_policy(request):
    ...
    return render(request,'apply_policy.html',{'policies':policies,'customer':customer})
...
def history(request):
    ...
    return render(request,'history.html',{'policies':policies,'customer':customer})
...
def ask_question(request):
    ...
    return render(request,'ask_question.html',{'questionForm':questionForm,'customer':customer})

def question_history(request):
    customer = models.Customer.objects.get(user_id=request.user.id)
    questions = CMODEL.Question.objects.all().filter(customer=customer)
    return render(request,'question_history.html',{'questions':questions,'customer':customer})
